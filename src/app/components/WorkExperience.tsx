    ...
    >
      {badges.map((badge) => (
        <li key={badge}>
          <Badge
            variant="secondary"
            className="align-middle text-xs print:px-1 print:text-[10px] print:leading-tight"
          >
            {badge}
          </Badge>
        </li>
...
function WorkExperienceItem({ work }: WorkExperienceItemProps) {
  ...
  return (
    <Card className="py-0 print:py-0">
      <CardHeader className="print:space-y-1">
        <div className="flex items-center justify-between gap-x-2 text-base ">
          <h2 className="inline-flex text-blue-500 print:text-blue-500 items-center  gap-x-1 font-semibold leading-none print:text-sm">
